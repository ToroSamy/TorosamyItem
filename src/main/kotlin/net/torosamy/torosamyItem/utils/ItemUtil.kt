...
class ItemUtil {
    companion object {
        const val TOROSAMY_ITEM_KEY: String = "TorosamyItem"
        val TOROSAMY_HASH_CODE_KEY: String = "HashCode"
        ...
        fun isTorosamyItem(itemStack: ItemStack?): Boolean {
            ...
            return ItemManager.items.containsKey(itemKey);
        }
        ...
        fun getItem(customItem: CustomItem,player: Player): ItemStack {
            ...
            if(customItem.color != null && itemMeta is LeatherArmorMeta) {
                itemMeta.setColor(Color.fromRGB(customItem.color!!.toInt(16)))
            }

            item.setItemMeta(itemMeta)

            NbtUtil.setString(item, TOROSAMY_ITEM_KEY, customItem.key)
            NbtUtil.setInteger(item, TOROSAMY_HASH_CODE_KEY, customItem.hashCode)
